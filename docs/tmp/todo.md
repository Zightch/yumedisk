...
- 基于现有 `tests/uia_*.ps1` 和当前最小 UI，完成一轮真实最小闭环验收：先确认运行环境里的 `AppKernel session` 能打开，再走通启动客户端、创建磁盘、查看列表与日志、删除磁盘、显式退出这一整条主线；如果中途失败，先把失败点收束为明确环境问题或实现问题，不提前做 UI 美化和扩展功能。
