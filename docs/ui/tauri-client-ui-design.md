...
- 左侧标题：`磁盘列表`。
- 标题旁展示数量，只显示数字，例如 `7`。
- 右侧先展示重扫按钮，再展示自动挂载统计，例如 `自动挂载 3 / 5`。
...
| 状态 | 展示 | 视觉 |
|---|---|---|
| mounted | `已挂载` | 成功色文本，卡片启用主题渐变 |
| unmounted | `未挂载` | 普通弱化标签 |
...
| `features` | 创建、挂载、拔出、删除、编辑、切主题等用户动作 |
...
- 内存盘挂载时，`media` 临时移交给 `BackendRust`；拔出时再回收到同一字段。
- 文件盘拔出或转为无效时，`media` 置空，不在宿主长期持有打开文件句柄。
- 文件盘重新挂载时，如果 `media` 为空，则按当前文件路径即时重建。
- 无效盘没有 `media` 实例。
...
- `mountDisk` 才是把盘真正交给 `BackendRust` 并创建系统可见盘的动作。
- `ejectDisk` 后，内存盘会回收同一份 `media`；文件盘会清空 `media`，下次挂载按路径重新打开。
- `rescanRuntimeDisks` 只作用于当前进程运行时磁盘列表，不重读配置，也不重新触发自动挂载。

启动与重试边界：

- 启动链固定为“`restore_client_state -> query_home_disk_list -> open_session -> rescan_runtime_disks -> 退出启动覆盖 -> 自动挂载`”。
- 会话失败后的重试链固定为“`open_session -> rescan_runtime_disks -> 自动挂载`”。
...
- 内存盘如果在当前 App 进程内拔出后重新挂载，可以继续使用原有内存数据；App 退出后这部分内存数据丢失。
...
- 文件盘未实现格式不能提交。
- 创建成功后只建立盘配置；内存盘会在运行时建立 `media`，文件盘不会自动挂载到系统。

### 16.4 挂载语义

- 手动点击挂载前，磁盘只存在于客户端列表和宿主内存中，不存在系统可见盘。
- 手动点击挂载时，`DiskRuntime` 当前唯一 `media` 字段必须被移交给 `BackendRust`。
- 手动点击拔出时，内存盘必须重新接回该 `media`；文件盘必须清空该 `media`。
- 内存盘在当前 App 常驻期间拔出后再次挂载，数据不丢。
- App 退出后，内存盘数据丢失；再次启动只能按配置重建空介质。
- 首次自动挂载只在程序首次进入主页且启动重扫成功后触发一次。
- 托盘重新显示主窗口不重复触发自动挂载。
...
4. 自动挂载只在首次主页启动链末尾执行，不在托盘重新显示时重复触发。
