...
因此本文档只描述当前唯一正确的结构，不继续保留旧的“认证隐式挂在 connection 上”“`SessionOpen` 顺带返回 metadata”“把宿主对象生命周期混进协议层”之类历史中间态。
...
- connection runtime 只表示一条可复用业务连接
- auth lane 只负责拿到 `auth_id`
- open lane 只负责把 `auth_id` 变成 `session_id`
- opened session 只表示已打开 session
- `SessionDescribe` 负责在已打开 session 上读取 metadata
- client disk object 显式记录 `disk_id + session + metadata`
...
- 一个 `auth_id` 只能消费一次
- 一个 connection 可以同时持有多个 `granted` 状态的 `auth_id`
...
client-gateway connection 当前只承接两类短生命周期过程：
...
client 侧接到这类确定性失效后，协议层只定义目标 session 已失效，不定义上层对象如何收束。
...
8. 客户端盘对象显式持有 `disk_id + session + metadata`。
9. 当前正式心跳只有两个方向：`client(connection) -> gateway : ConnHeartbeat`，`gateway -> storer : LinkHeartbeat`。
10. `whole` 对 client 必须走完整 `Hello -> transport -> auth -> session -> metadata -> data plane` 主链，但 route 固定为本地唯一 `disk_id`。
11. route 或 connection 失效时，gateway 负责清理授权与会话；协议层不定义上层对象的收束策略。
12. 正式文档不保留历史兼容和过渡叙述。
