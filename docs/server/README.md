...
它承接 `docs/network` 没有定义的 server 侧策略，包括：
...
challenge token 的内部编码、随机源和连接绑定细节属于 gateway 实现，不在 `docs/network` 展开。
...
- `docs/network` 不定义统一 `busy` 业务语义
- 当前项目把 open 失败视为 server 自己的 reject / failure
- 若某个 storer 策略选择拒绝新的 open，gateway 直接把 reject 回给 client
